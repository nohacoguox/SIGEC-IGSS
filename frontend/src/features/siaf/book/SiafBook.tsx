// frontend/src/features/siaf/book/SiafBook.tsx
...
import { parseMarcadoresBitacora, SiafMarcaBitacora } from '../../../utils/siafBitacora';
import type {
  Area,
  BitacoraEntry,
  CatalogoOrigen,
...
import {
  findUnidadEnCatalogo,
  getValidationError,
  labelUnidadMedica,
} from './utils';
import SiafOrtografiaDialog from './components/SiafOrtografiaDialog';
import SiafPreviewDialog from './components/SiafPreviewDialog';
import SiafMarcasViewerDialog from './components/SiafMarcasViewerDialog';
import SiafAdjuntoViewerDialog from './components/SiafAdjuntoViewerDialog';
import SiafBitacoraCard from './components/SiafBitacoraCard';
...
const SiafBook: React.FC = () => {
  ...
  const [unidadesMedicas, setUnidadesMedicas] = useState<UnidadMedicaCatalogo[]>([]);
  ...
  const [ortografiaSugerencias, setOrtografiaSugerencias] = useState<OrtografiaSugerencia[]>([]);
  ...
  const handleDownloadAdjunto = async (doc: ViewingDoc) => {
    try {
      const res = await api.get(`/siaf/adjuntos/${doc.id}/descargar`, { responseType: 'blob' });
  ...
  return (
    <>
          ...
          >
            <Paper sx={{ p: 4, borderRadius: 3 }} elevation={3}>
              ...
              {id && bitacora.length > 0 && (
                <SiafBitacoraCard bitacora={bitacora} onVerMarcas={handleVerMarcasEnPdf} />
              )}
      ...
      <SiafOrtografiaDialog
        open={ortografiaOpen}
        sugerencias={ortografiaSugerencias}
        elegidas={ortografiaElegidas}
        texto={ortografiaTexto}
        onClose={() => setOrtografiaOpen(false)}
        onElegidaChange={(idx, value) =>
          setOrtografiaElegidas((prev) => ({ ...prev, [idx]: value }))
        }
        onAplicar={(textoCorregido) => {
          setJustificacion(textoCorregido.slice(0, 500));
      ...
      <SiafPreviewDialog
        ...
        onCloseAndNavigate={() => navigate('/siaf-book')}
      />

      <SiafMarcasViewerDialog
      ...
      <SiafAdjuntoViewerDialog
        open={viewerOpen}
        viewingDoc={viewingDoc}
        onClose={cerrarAdjuntoViewer}
        onDownload={handleDownloadAdjunto}
      />
