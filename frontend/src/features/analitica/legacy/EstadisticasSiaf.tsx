...
interface EstadisticasSiafProps {
  /** Pesta├▒a a mostrar al cargar (0 = Tiempos SIAF, 1 = Motivos, 2 = Piloto PG2) */
  tabInicial?: 0 | 1 | 2;
  /** Si es true, no se muestran las pesta├▒as y solo el contenido de tabInicial */
  ocultarTabs?: boolean;
}
...
const EstadisticasSiaf: React.FC<EstadisticasSiafProps> = ({ tabInicial = 0, ocultarTabs = false }) => {
    ...
    Promise.all([
      ...
      .catch((err) => {
        ...
        const fallback = err.response
          ? (msg || 'Error al cargar estad├¡sticas.')
          : 'No se pudo conectar con el servidor. Verifique que el backend est├® en ejecuci├│n (puerto 3001).';
  ...
  const kpiCards = [
    {
      title: 'Tiempo hasta autorizaci├│n',
      value: data.promedioAutorizacionHoras != null ? formatHoras(data.promedioAutorizacionHoras) : 'ÔÇö',
      subtitle: 'Desde generaci├│n hasta autorizaci├│n',
    ...
    {
      title: 'Tiempo de revisi├│n (total)',
      value: data.promedioRevisionHoras != null ? formatHoras(data.promedioRevisionHoras) : 'ÔÇö',
      subtitle: 'Hasta autorizaci├│n o rechazo',
    ...
    {
      title: 'Tiempo de correcci├│n',
      value: data.promedioCorreccionHoras != null ? formatHoras(data.promedioCorreccionHoras) : 'ÔÇö',
      subtitle: 'Desde rechazo hasta correcci├│n',
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto' }}>
      ...
      <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap' }}>
        <FormControl size="small" sx={{ minWidth: 200 }}>
          ...
          <Select value={dias} label="Per├¡odo" onChange={(e) => setDias(Number(e.target.value))}>
            <MenuItem value={30}>├Ültimos 30 d├¡as</MenuItem>
            <MenuItem value={90}>├Ültimos 90 d├¡as</MenuItem>
            <MenuItem value={180}>├Ültimos 6 meses</MenuItem>
            <MenuItem value={365}>├Ültimo a├▒o</MenuItem>
          </Select>
        </FormControl>
      ...
      {!ocultarTabs && (
        <Tabs value={tabEstadisticas} onChange={(_, v) => setTabEstadisticas(v)} sx={{ mb: 3, borderBottom: 1, borderColor: 'divider' }}>
          <Tab label="SIAF ÔÇö Tiempos de revisi├│n, autorizaci├│n y correcci├│n" id="estad-tab-0" aria-controls="estad-panel-0" />
          <Tab label="Motivos de rechazo m├ís frecuentes" id="estad-tab-1" aria-controls="estad-panel-1" />
          <Tab label="Piloto PG2 ÔÇö Variables de investigaci├│n" id="estad-tab-2" aria-controls="estad-panel-2" />
        </Tabs>
      )}
          ...
          <Box>
                    ...
                    <Table size="small">
                      <TableHead>
                        <TableRow sx={{ bgcolor: IGSS_COLORS.azulOscuro }}>
                          ...
                          <TableCell sx={{ fontWeight: 700, py: 1.5, color: '#fff' }} align="right">Prom. revisi├│n</TableCell>
                          <TableCell sx={{ fontWeight: 700, py: 1.5, color: '#fff' }} align="right">Prom. autorizaci├│n</TableCell>
                          <TableCell sx={{ fontWeight: 700, py: 1.5, color: '#fff' }} align="right">Prom. correcci├│n</TableCell>
                      ...
                      <TableBody>
                        {chartData.map((row) => (
                          ...
                          >
                            ...
                            <TableCell align="right" sx={{ py: 1.5 }}>{row.revision > 0 ? formatHoras(row.revision) : 'ÔÇö'}</TableCell>
                            <TableCell align="right" sx={{ py: 1.5 }}>{row.autorizacion > 0 ? formatHoras(row.autorizacion) : 'ÔÇö'}</TableCell>
                            <TableCell align="right" sx={{ py: 1.5 }}>{row.correccion > 0 ? formatHoras(row.correccion) : 'ÔÇö'}</TableCell>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 2, mb: 1.5, flexWrap: 'wrap' }}>
                            <Box>
                              <Typography variant="subtitle1" fontWeight={800} sx={{ color: IGSS_COLORS.azulOscuro, letterSpacing: '-0.01em' }}>
                                Revisi├│n y autorizaci├│n
                              </Typography>
                              <Typography variant="caption" color="text.secondary">
                                Promedio semanal en horas ┬À comparaci├│n directa
                              </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', gap: 1.25 }}>
                              <LegendPill color={IGSS_COLORS.azulClaro} label="Revisi├│n" />
                              <LegendPill color={IGSS_COLORS.azul} label="Autorizaci├│n" />
                            </Box>
                          </Box>
                      ...
                      <motion.div custom={1} variants={fadeUp} initial="hidden" animate="show">
                        <Paper elevation={0} sx={chartPanelSx}>
                          <Box sx={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 2, mb: 1.5, flexWrap: 'wrap' }}>
                            ...
                            <LegendPill color={IGSS_COLORS.verde} label="Correcci├│n" />
                          </Box>
                          <Box sx={{ width: '100%', height: 340 }}>
            ...
            {chartData.length === 0 && (
              <Alert severity="info" sx={{ borderRadius: 2 }}>
                No hay datos de revisi├│n o correcci├│n en el per├¡odo seleccionado. Las estad├¡sticas se actualizar├ín cuando existan SIAFs autorizados, rechazados o corregidos.
              </Alert>
            )}
          </Box>
        )}
        ...
        {tabActual === 1 && motivosData && (
          <Box>
            <Typography variant="subtitle1" fontWeight="600" color="text.primary" sx={{ mb: 1, display: 'block' }}>
              Motivos de rechazo m├ís frecuentes
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Rechazos en el per├¡odo seleccionado clasificados por categor├¡a. Al rechazar un SIAF, Direcci├│n Departamental puede elegir la categor├¡a para que estas estad├¡sticas se actualicen.
            </Typography>
            {motivosData.total === 0 ? (
              <Alert severity="info" sx={{ borderRadius: 2 }}>
                No hay rechazos registrados en este per├¡odo, o a├║n no se han clasificado motivos. Los rechazos nuevos pueden incluir una categor├¡a al confirmar el rechazo.
              </Alert>
            ) : (
                ...
                <Paper elevation={0} sx={{ ...chartPanelSx, p: 3 }}>
                  ...
                  {motivosData.sinClasificar > 0 && (
                    <Typography variant="caption" color="text.secondary" sx={{ mt: 1.5, display: 'block' }}>
                      {motivosData.sinClasificar} rechazo(s) sin categor├¡a asignada (anteriores a esta funcionalidad).
                    </Typography>
                  )}
                </Paper>
              </motion.div>
        ...
        {tabActual === 2 && tesisData && (
          <Box>
            <Typography variant="subtitle1" fontWeight="700" sx={{ color: IGSS_COLORS.azulOscuro, mb: 0.5 }}>
              Seguimiento del piloto: l├¡nea base vs. SIGEC-IGSS
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Los indicadores del piloto se calculan desde expedientes que tienen N├║mero SIAF y O.C. registrados. La l├¡nea base corresponde a los instrumentos V1ÔÇôV4 verificados (n = 25).
            </Typography>
            {tesisData.muestra.total < tesisData.muestra.meta && (
              <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                Muestra piloto actual: {tesisData.muestra.total} de {tesisData.muestra.meta} casos. Faltan {tesisData.muestra.pendientesParaMeta} expedientes identificados para completar la meta metodol├│gica.
              </Alert>
            )}
            <Grid container spacing={2.5}>
              <Grid item xs={12} md={6}>
                <PilotCard
                  title="V1 ┬À Eficiencia"
                  baseline={`Ciclos: ${tesisData.lineaBase.eficiencia.promedioCiclos.toFixed(2)} ┬À Devueltos: ${tesisData.lineaBase.eficiencia.devueltosPorcentaje.toFixed(0)}%`}
                  pilot={`Ciclos: ${formatMetric(tesisData.piloto.eficiencia.promedioCiclos)} ┬À Devueltos: ${formatPercent(tesisData.piloto.eficiencia.devueltosPorcentaje)}`}
                  detail={`Casos que pasaron al mes siguiente: ${tesisData.piloto.eficiencia.pasaronMes}`}
                />
              </Grid>
              <Grid item xs={12} md={6}>
                <PilotCard
                  title="V2 ┬À Calidad documental"
                  baseline={`Observaciones: ${tesisData.lineaBase.calidad.observacionesPromedio.toFixed(2)} ┬À Rechazos/100: ${tesisData.lineaBase.calidad.rechazosPor100.toFixed(0)}`}
                  pilot={`Observaciones: ${formatMetric(tesisData.piloto.calidad.observacionesPromedio)} ┬À Rechazos/100: ${formatMetric(tesisData.piloto.calidad.rechazosPor100)}`}
                  detail="Las observaciones se cuentan desde los motivos documentados por DAF."
                />
              </Grid>
              <Grid item xs={12} md={6}>
                <PilotCard
                  title="V3 ┬À Trazabilidad"
                  baseline={`Criterio global T1ÔÇôT5: ${tesisData.lineaBase.trazabilidad.cumplePorcentaje.toFixed(0)}% ┬À B├║squeda: ${tesisData.lineaBase.trazabilidad.minutosBusqueda.toFixed(2)} min`}
                  pilot={`Criterio global T1ÔÇôT5: ${formatPercent(tesisData.piloto.trazabilidad.cumplePorcentaje)} ┬À Versiones distinguibles: ${formatPercent(tesisData.piloto.trazabilidad.versionesDistinguiblesPorcentaje)}`}
              ...
              <Grid item xs={12} md={6}>
                <PilotCard
                  title="V4 ┬À Tiempos"
                  baseline={`Ciclo: ${tesisData.lineaBase.tiempos.cicloDiasHabiles.toFixed(2)} d├¡as h├íbiles ┬À 1.┬¬ respuesta: ${tesisData.lineaBase.tiempos.primeraRespuestaDiasHabiles.toFixed(2)}`}
                  pilot={`Ciclo: ${formatMetric(tesisData.piloto.tiempos.cicloDiasHabiles)} d├¡as h├íbiles ┬À 1.┬¬ respuesta: ${formatMetric(tesisData.piloto.tiempos.primeraRespuestaDiasHabiles)}`}
                  detail="Se calcula desde el env├¡o a revisi├│n DAF hasta la primera resoluci├│n y aprobaci├│n."
                />
              </Grid>
            </Grid>
          </Box>
...
function formatMetric(value: number | null): string {
  return value == null ? 'ÔÇö' : value.toFixed(2);
}

function formatPercent(value: number | null): string {
  return value == null ? 'ÔÇö' : `${value.toFixed(0)}%`;
}
...
function PilotCard({ title, baseline, pilot, detail }: { title: string; baseline: string; pilot: string; detail: string }) {
  return (
    <Paper elevation={0} sx={{ p: 2.25, border: '1px solid', borderColor: 'divider', borderRadius: 2.5, height: '100%' }}>
      ...
      <Typography variant="caption" color="text.secondary" display="block">L├¡nea base (n = 25)</Typography>
...
/** Gr├ífico: revisi├│n y autorizaci├│n por semana (horas) */
function ChartHoras({
  data,
  formatHoras,
}: {
...
/** Gr├ífico: tiempo de correcci├│n por semana (minutos) */
...
/** Gr├ífico de barras horizontales: motivos de rechazo */
