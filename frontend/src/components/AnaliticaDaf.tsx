...
type Analytics = {
  ...
  motivos?: Array<{ motivo: string; cantidad: number }>;
  cierreMensual?: CierreMes[];
  trazabilidad?: Analytics['porSiaf'] extends infer P ? P extends { trazabilidad: infer T } ? T : never : never;
};
...
const AnaliticaDaf: React.FC = () => {
  ...
  const tiempos = data.porSiaf?.tiempos ?? data.tiempos;
  const ciclos = data.porSiaf?.ciclos ?? data.ciclos ?? [];
  const motivos = data.porSiaf?.motivos ?? data.motivos ?? [];
  const trazabilidad = data.porSiaf?.trazabilidad ?? data.trazabilidad ?? [];
  const casos = data.porSiaf?.casos ?? [];
  ...
  return (
    <Box sx={{
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}>
          <Grid container spacing={2.25} sx={{ mb: 2.5 }}>
            <Grid item xs={12} sm={6} lg={3}>
              <KpiCard label="SIAFs" value={resumen.total} note="Casos del período" icon={<FactCheck />} color={IGSS_COLORS.azulOscuro} />
            </Grid>
            <Grid item xs={12} sm={6} lg={3}>
                ...
                <Box sx={{ height: 340, mt: 2 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={cierre} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
                      ...
                      <Bar dataKey="aprobados" name="Aprobados" fill="url(#gradAprobadosSiaf)" radius={[6, 6, 0, 0]} barSize={18} />
                      <Bar dataKey="rechazadosAlCierre" name="Rechazados al cierre" fill="url(#gradRechazoSiaf)" radius={[6, 6, 0, 0]} barSize={18} />
              ...
              <Paper elevation={0} sx={{ ...panelSx, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: IGSS_COLORS.fondoClaro }}>
                  <Typography variant="body2" color="text.secondary">
                    {resumen.pendientesRevisionDaf} SIAF(s) en revisión DAF
                  </Typography>
                </Box>
              </Paper>
            </Grid>
      ...
      {tab === 1 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}>
          {!filters.itemId && (
            <Alert severity="info" sx={{ mb: 2.25, borderRadius: 2 }}>Seleccione un SIAF para ver su trazabilidad.</Alert>
          )}
                  ...
                  <TableBody>
                    {casos.map((caso) => (
                      <TableRow key={caso.id} hover sx={{ '& td': { borderBottomColor: gridStroke } }}>
                        <TableCell><Typography variant="body2" fontWeight={700}>{caso.correlativo}</Typography></TableCell>
                ...
                <Box sx={{ height: 260, mt: 1.5 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={tiemposSerie} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
                      <defs>
                        <linearGradient id="gradTiemposSiaf" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Tooltip content={<ChartTooltip />} />
                      <Area type="monotone" dataKey="horas" name="Horas" stroke="none" fill="url(#gradTiemposSiaf)" />
                  ...
                  <Box sx={{ height: Math.max(340, motivos.length * 52), mt: 2 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={motivosConPorcentaje} layout="vertical" margin={{ top: 4, right: 56, left: 8, bottom: 4 }}>
                        <defs>
                          <linearGradient id="gradMotivosSiaf" x1="0" y1="0" x2="1" y2="0">
